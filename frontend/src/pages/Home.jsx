...
const FormContainer = ({ children, isLeft, color, activeTab }) => (
  ...
  return (
    <div className={`bg-white rounded-xl shadow-md p-3 md:p-5 border-l-4 ${cardBorder} min-w-[210px] md:min-w-[300px] w-[210px] md:w-[300px] flex-shrink-0 transition-all`}>
      ...
      <div className="space-y-1.5 pt-2 border-t border-gray-50">
        ...
        <button onClick={onView} className={`w-full ${isBudgeted ? 'bg-indigo-600' : 'bg-lime-500'} text-white py-1.5 rounded-lg text-[10px] md:text-sm mt-1 flex items-center justify-center font-medium`}><Eye size={12} className="mr-1.5" /> View</button>
      </div>
    </div>
  );
};
    ...
    <div className="w-full min-h-screen bg-gray-50 p-3 md:p-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === "withAmount" ? (
          <FormContainer isLeft={true} color="indigo" activeTab={activeTab}>
          ...
          <FormContainer isLeft={false} color="lime" activeTab={activeTab}>
            ...
            <div className="flex space-x-3 min-w-max">
              {(activeTab === "withAmount" ? expenses : withoutAmountExpenses).map((expense) => (
                <HistoryCardRevised
                  key={expense._id}
                  expense={expense}
                  isBudgeted={activeTab === "withAmount"}
                  handleDelete={activeTab === "withAmount" ? handleDeleteExpense : handleDeleteWithoutAmountExpense}
                  onView={() => navigate(activeTab === "withAmount" ? `/with-ammount/${expense._id}` : `/without-amount/${expense._id}`)}
                />
              ))}
            </div>
          </div>
